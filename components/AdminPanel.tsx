    ...
    reader.onload = async (event) => {
      try {
        ...
        if (normalizedData.length === 0) {
          setError("No valid records found (शून्य रेकॉर्ड सापडले). Excel Check kara: 'Seat No' and 'Student Name' column names.");
        } else {
          // Upload to Firestore
          await uploadMasterRecordsBatch(currentUser.id, normalizedData, (processed, total) => {
        ...
        setError(`Upload Failed (अपलोड अयशस्वी). Check Internet or File Format. Error: ${err.message}`);
      } finally {
        setLoading(false);
  ...
  const handleAddressFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (normalizedData.length === 0) {
          setError("No valid address records found. Please ensure Excel has headers like 'College Code' and 'Address'.");
        } else {
          // Upload to Firestore
          await uploadAddressesBatch(currentUser.id, normalizedData, (processed, total) => {
              const pct = Math.round((processed / total) * 100);
              setUploadProgress(pct);
          });
        ...
        setError(`Failed to upload addresses. Error: ${err.message || "Unknown error"}`);
      } finally {
        setLoading(false);
        if (inputElement) inputElement.value = ''; // Reset input
      }
      ...
      <div className="bg-white p-8 rounded-b-xl shadow-md border border-gray-200 border-t-0 mt-0">
        ...
        {activeTab === 'MASTER' && (
          <>
            ...
            <p className="text-gray-500 mb-8">
              Upload an Excel file containing student details to enable auto-fill in the application form.
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 flex flex-col items-center justify-center text-center hover:bg-gray-50 transition-colors">
                    ...
                    <label className="cursor-pointer">
                        ...
                        <input type="file" className="hidden" accept=".xlsx, .xls" onChange={handleMasterFileUpload} />
                    </label>
                    ...
                    <div className="space-y-4">
                        ...
                        {loading && (
                            <div className="mt-4">
                                ...
                                <p className="text-xs text-gray-500 mt-1 text-center">Please wait, do not close this tab.</p>
                            </div>
                        )}
                        {error && <div className="flex items-start gap-2 text-red-600 text-sm bg-red-50 p-3 rounded-lg"><AlertCircle size={16} className="mt-0.5 shrink-0" />{error}</div>}
        ...
        {activeTab === 'ADDRESS' && (
          <>
            ...
            <p className="text-gray-500 mb-8">
              Upload an Excel file containing college addresses and emails.
            </p>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 flex flex-col items-center justify-center text-center hover:bg-gray-50 transition-colors">
                    ...
                    <label className="cursor-pointer">
                        <span className="bg-emerald-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-emerald-700 transition-colors inline-block mb-2">
                            Select Address Excel
                        </span>
                        <input type="file" className="hidden" accept=".xlsx, .xls" onChange={handleAddressFileUpload} />
                    </label>
                    <p className="text-xs text-gray-400 mt-2">Supported formats: .xlsx, .xls</p>
